...
void findWaitingResTime(struct Process proc[], int n, int wt[],int rest[])
{
	//Arrays for remaining time
	int rt[n],io[n],iolist[n],k=0;
...
void findavgTime(struct Process proc[], int n)
{
	...
	int wt[n], tat[n], rest[n], total_wt = 0, total_tat = 0,total_rest=0;
	for(int i=0;i<n;i++)
	{
        wt[i]=tat[i]=rest[i]=0;
	}
